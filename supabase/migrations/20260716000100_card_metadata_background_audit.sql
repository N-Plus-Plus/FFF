alter table public.shows
  ...
  add column if not exists end_year integer check (end_year is null or end_year >= 1800);

update public.shows
set total_season_count = coalesce(
      total_season_count,
      nullif(metadata->>'total_season_count', '')::integer,
      nullif(metadata->>'tvmaze_season_count', '')::integer,
      nullif(metadata #>> '{last_refresh,total_season_count}', '')::integer,
      nullif(metadata #>> '{last_refresh,upstream,tvmaze_season_count}', '')::integer
    ),
    end_year = coalesce(
      end_year,
      nullif(metadata->>'end_year', '')::integer,
      nullif(metadata->>'tvmaze_end_year', '')::integer,
      nullif(metadata #>> '{last_refresh,end_year}', '')::integer,
      nullif(metadata #>> '{last_refresh,upstream,tvmaze_end_year}', '')::integer
    )
...
begin
  ...
  select * into v_show from public.shows where normalized_imdb_id = v_imdb_id limit 1;

  if v_show.id is null then
    insert into public.shows (
      imdb_id, title, release_year, end_year, title_type, provider_title_type,
      series_status, total_season_count, total_episode_count, total_runtime_minutes,
      metadata_provider, provider_record_id, tvdb_record_id, tvdb_metadata_retrieved_at,
      metadata_source_provenance, metadata_retrieved_at, metadata_refresh_attempted_at,
      metadata_refresh_succeeded_at, metadata_refresh_status, poster_url, poster_source_url,
      background_url, poster_retrieval_status, poster_refresh_status, poster_updated_at,
      disambiguation, metadata, first_enrolled_by
    )
    values (
      v_imdb_id, btrim(p_title), p_release_year, nullif(p_metadata->>'end_year', '')::integer,
      v_provider_title_type, v_provider_title_type,
      nullif(btrim(coalesce(p_metadata->>'series_status', '')), ''),
      nullif(p_metadata->>'total_season_count', '')::integer,
    ...
    set title = coalesce(nullif(btrim(p_title), ''), title),
        ...
        end_year = coalesce(nullif(p_metadata->>'end_year', '')::integer, end_year),
        ...
        total_season_count = coalesce(nullif(p_metadata->>'total_season_count', '')::integer, total_season_count),
        ...
        tvdb_metadata_retrieved_at = case when nullif(btrim(coalesce(p_metadata->>'tvdb_record_id', '')), '') is not null then v_now else tvdb_metadata_retrieved_at end,
        metadata_source_provenance = case when v_source_provenance = '{}'::jsonb then metadata_source_provenance else v_source_provenance end,
        ...
        poster_retrieval_status = case when poster_storage_path is null then coalesce(nullif(btrim(coalesce(p_metadata->>'poster_retrieval_status', '')), ''), poster_retrieval_status) else poster_retrieval_status end,
  ...
  select exists (
    select 1 from public.show_nominations n
    where n.user_id = v_user.id and n.show_id = v_show.id and n.withdrawn_at is null
...
begin
  update public.shows
  set title = case when v_success then coalesce(nullif(btrim(coalesce(p_metadata->>'title', '')), ''), title) else title end,
      release_year = case when v_success then coalesce(nullif(p_metadata->>'release_year', '')::integer, release_year) else release_year end,
      end_year = case when v_success then coalesce(nullif(p_metadata->>'end_year', '')::integer, end_year) else end_year end,
      provider_title_type = case when v_success then coalesce(nullif(btrim(coalesce(p_metadata->>'provider_title_type', '')), ''), provider_title_type) else provider_title_type end,
      series_status = case when v_success then coalesce(nullif(btrim(coalesce(p_metadata->>'series_status', '')), ''), series_status) else series_status end,
      total_season_count = case when v_success then coalesce(nullif(p_metadata->>'total_season_count', '')::integer, total_season_count) else total_season_count end,
      total_episode_count = case when v_success then coalesce(nullif(p_metadata->>'total_episode_count', '')::integer, total_episode_count) else total_episode_count end,
      total_runtime_minutes = case when v_success and not v_runtime_is_estimate then coalesce(nullif(p_metadata->>'total_runtime_minutes', '')::integer, total_runtime_minutes) else total_runtime_minutes end,
      metadata_provider = case when v_success then coalesce(nullif(btrim(coalesce(p_metadata->>'metadata_provider', '')), ''), metadata_provider) else metadata_provider end,
      provider_record_id = case when v_success then coalesce(nullif(btrim(coalesce(p_metadata->>'provider_record_id', '')), ''), provider_record_id) else provider_record_id end,
      tvdb_record_id = case when v_success then coalesce(nullif(btrim(coalesce(p_metadata->>'tvdb_record_id', '')), ''), tvdb_record_id) else tvdb_record_id end,
      tvdb_metadata_retrieved_at = case when v_success and nullif(btrim(coalesce(p_metadata->>'tvdb_record_id', '')), '') is not null then v_now else tvdb_metadata_retrieved_at end,
      metadata_retrieved_at = case when v_success then coalesce(nullif(p_metadata->>'metadata_retrieved_at', '')::timestamptz, v_now) else metadata_retrieved_at end,
      poster_source_url = case when v_success then coalesce(nullif(btrim(coalesce(p_metadata->>'poster_source_url', '')), ''), poster_source_url) else poster_source_url end,
      background_url = case when v_success then coalesce(nullif(btrim(coalesce(p_metadata->>'background_url', '')), ''), background_url) else background_url end,
      metadata_source_provenance = case when v_success then coalesce(p_metadata->'source_provenance', metadata_source_provenance) else metadata_source_provenance end,
      metadata = case when v_success then jsonb_strip_nulls(metadata || jsonb_build_object('last_refresh', p_metadata)) else metadata end,
      ...
      metadata_refresh_failure_category = case when v_success then null else nullif(btrim(coalesce(p_metadata->>'metadata_refresh_failure_category', 'failed')), '') end
  where id = p_show_id
  returning * into v_show;
...
drop function if exists public.get_board(text);
...
begin
  ...
  where s.admin_removed_at is null
    and exists (select 1 from public.show_nominations n where n.show_id = s.id and n.withdrawn_at is null)
    and exists (select 1 from public.user_show_rankings r where r.show_id = s.id and r.rank_position is not null);
  ...
  while coalesce(array_length(v_remaining, 1), 0) > 0 loop
    ...
    with active_users as (
      select id from public.app_users where is_active = true and token_revoked_at is null
    ),
    show_row as (
      select
        s.*,
        (select count(*)::integer from public.show_nominations n where n.show_id = s.id and n.withdrawn_at is null) as active_nomination_count,
        (select count(*)::integer from public.user_show_rankings r join active_users au on au.id = r.user_id where r.show_id = s.id and r.rank_position is not null) as ranked_active_user_count,
    ...
    select
      ...
      show_row.active_user_count > 0 and show_row.ranked_active_user_count = show_row.active_user_count
    from show_row;
