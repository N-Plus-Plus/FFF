...
type ImdbRequest =
  ...
  | { action: "verify-backgrounds"; token?: string; items?: BackgroundAuditItem[] };

type BackgroundAuditItem = {
  showId?: string;
  imdbId?: string;
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (body.action === "verify-backgrounds") {
      const items = Array.isArray(body.items) ? body.items : [];
      const result = await verifyBackgrounds(items);
      return json({ user: userResult.user, ...result }, 200, cors);
    }

    return json({ error: "invalid_action", message: "Use action search, lookup, enroll, or verify-backgrounds." }, 400, cors);
  } catch (error) {
...
function corsHeaders(origin: string): HeadersInit {
  const allowed = (Deno.env.get("ALLOWED_ORIGINS") || "")
    ...
    .map((item) => item.trim())
    .filter(Boolean);
  const allowOrigin = allowed.includes(origin) || isLoopbackOrigin(origin)
    ? origin
...
async function verifyBackgrounds(items: BackgroundAuditItem[]) {
  const supabaseUrl = Deno.env.get("SUPABASE_URL") || "";
  const serviceKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") || "";
  ...
  const adapter = createImdbProviderAdapter();
  const serviceClient = createClient(supabaseUrl, serviceKey, {
    global: { headers: { "x-application-name": "fff-background-audit" } }
  });
  const updated: string[] = [];
  const seen = new Set<string>();

  for (const item of items.slice(0, 30)) {
    const showId = typeof item.showId === "string" ? item.showId.trim() : "";
    const imdbId = normalizeImdbId(item.imdbId || "");
    if (!showId || !imdbId || seen.has(showId)) {
      continue;
    }
    seen.add(showId);
    ...
    const { error } = await serviceClient.rpc("admin_update_show_metadata", {
      p_show_id: showId,
      p_metadata: {
        ...providerMetadata(latest),
        imdb_id: latest.imdb_id,
        title: latest.title,
        release_year: latest.release_year,
...
function createImdbProviderAdapter() {
  ...
  return {
    ...
    async search(query: string): Promise<NormalizedTitle[]> {
      ...
      const items = extractTvmazeSearchItems(payload);
      const results = await Promise.all(items.map(async (item) => {
        const episodes = await fetchTvmazeEpisodes(item, timeoutMs).catch(() => null);
        return normalizeTvmazeShow(item, episodes);
      }));
      return results.filter(isTelevisionTitle);
    },
...
async function fetchTvmazeImages(show: Record<string, unknown>, timeoutMs: number): Promise<Record<string, unknown>[] | null> {
  ...
  const payload = await fetchProviderJson(`${TVMAZE_BASE_URL}/shows/${tvmazeId}/images`, "", "", timeoutMs);
  return Array.isArray(payload) ? payload.filter(isRecord) : null;
}
...
function normalizeTvdbTitle(item: Record<string, unknown>, canonicalImdbId: string): NormalizedTitle {
  ...
  return {
    ...
    end_year: yearValue(item.end_year) || yearValue(item.endYear) || yearValue(item.ended) || null,
