...
function normalizeShow(show, config = null) {
  ...
  return {
    ...
    endYear: show.end_year || show.endYear || show.ended_year || show.endedYear || show.final_year || show.finalYear || show.metadata?.end_year || show.metadata?.endYear || show.metadata?.tvmaze_end_year || null,
    ...
    tvmazeRating: show.tvmaze_rating ?? show.tvmazeRating ?? null,
...
function normalizeBoard(board, config = null) {
  return {
    ...
    entries: (board?.entries || []).map((entry) => ({
      ...
      endYear: entry.end_year || entry.endYear || entry.ended_year || entry.endedYear || entry.final_year || entry.finalYear || entry.metadata?.end_year || entry.metadata?.endYear || entry.metadata?.tvmaze_end_year || null,
