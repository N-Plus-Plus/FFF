...
async function retainArtwork(showId, item, env) {
  const retained = { poster: false, cardArt: false };
  for (const [kind, source, column] of [["posters", item.poster_source_url, "poster_storage_path"], ["card-art", item.card_art_source_url, "card_art_storage_path"]]) {
    ...
    if (kind === "posters") retained.poster = true;
    else retained.cardArt = true;
  }
  return retained;
}
async function verifyBackgrounds(items, env) { const updated = [], seen = new Set(); for (const item of Array.isArray(items) ? items.slice(0, 30) : []) { const id = String(item?.showId || "").trim(), imdb = String(item?.imdbId || "").trim().toLowerCase(); if (!id || !/^tt\d{7,10}$/.test(imdb) || seen.has(id)) continue; seen.add(id); const existing = await env.DB.prepare("SELECT * FROM shows WHERE id=? AND imdb_id=?").bind(id, imdb).first(); if (!existing) continue; const show = await provider(`${TVMAZE}/lookup/shows?imdb=${imdb}`).catch(() => null); if (!show) continue; const fresh = await normalizeTvmaze(show); if (!fresh.card_art_source_url || fresh.card_art_type === "placeholder") continue; const changed = existing.card_art_source_url !== fresh.card_art_source_url || existing.card_art_type !== fresh.card_art_type || Number(existing.card_art_width || 0) !== Number(fresh.card_art_width || 0) || Number(existing.card_art_height || 0) !== Number(fresh.card_art_height || 0); if (!changed) continue; const retained = await retainArtwork(id, fresh, env).catch(() => null); if (!retained?.cardArt) continue; await env.DB.prepare("UPDATE shows SET card_art_source_url=?,card_art_type=?,card_art_width=?,card_art_height=?,background_url=?,metadata=?,updated_at=? WHERE id=?").bind(fresh.card_art_source_url,fresh.card_art_type,fresh.card_art_width,fresh.card_art_height,fresh.background_url,JSON.stringify(fresh.metadata),now(),id).run(); updated.push(id); } if (updated.length) await bump(env.DB); return { updatedCount: updated.length, updated }; }
...
function selectArt(show, images) { const candidates = []; for (const type of ["background", "banner"]) for (const image of Array.isArray(images) ? images : []) { if (image?.type !== type) continue; const source = type === "background" ? image?.resolutions?.original : image?.resolutions?.original || image?.resolutions?.medium; if (source?.url) candidates.push({ type, url: source.url, width: Number(source.width) || null, height: Number(source.height) || null, main: !!image.main }); } if (candidates.length) return candidates.sort((a,b) => { const da = a.width && a.height ? Math.abs(Math.log((a.width / a.height) / 5)) : Infinity, db = b.width && b.height ? Math.abs(Math.log((b.width / b.height) / 5)) : Infinity; return da - db || Number(b.main) - Number(a.main) || ((b.width || 0) * (b.height || 0)) - ((a.width || 0) * (a.height || 0)); })[0]; const poster = show.image?.original || show.image?.medium; return poster ? { type: "poster", url: poster, width: null, height: null } : { type: "placeholder", url: null, width: null, height: null }; }
